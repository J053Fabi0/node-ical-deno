...
try {
  ...
  if (unresolved.length > 0) {
    console.warn(
      `windowsZones: ${unresolved.length} legacy alias(es) could not be resolved to a canonical Windows ID -> IANA mapping.`
    );
    for (const { label, windowsId } of unresolved) {
      console.warn(
        `  - Skipped legacy label ${JSON.stringify(label)} -> ${JSON.stringify(windowsId)} (no primary IANA found)`
      );
    }
